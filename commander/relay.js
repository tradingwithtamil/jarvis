...
function queue(deviceId,action,args){const id=crypto.randomUUID();state.commands[id]={id,deviceId,action,args:args||{},status:'queued',createdAt:new Date().toISOString()};persist();return state.commands[id]}
...
const tools=[
{name:'list_devices',description:'List Jarvis Commander devices.',inputSchema:{type:'object',properties:{}}},
{name:'run_command',description:'Run PowerShell/CMD/shell command.',inputSchema:{type:'object',properties:{device_id:{type:'string'},command:{type:'string'},timeout_ms:{type:'integer'}},required:['device_id','command']}},
{name:'read_file',description:'Read a UTF-8 file.',inputSchema:{type:'object',properties:{device_id:{type:'string'},path:{type:'string'}},required:['device_id','path']}},
{name:'write_file',description:'Write a UTF-8 file.',inputSchema:{type:'object',properties:{device_id:{type:'string'},path:{type:'string'},content:{type:'string'},mode:{type:'string'}},required:['device_id','path','content']}},
{name:'list_directory',description:'List directory.',inputSchema:{type:'object',properties:{device_id:{type:'string'},path:{type:'string'}},required:['device_id','path']}},
{name:'system_status',description:'Get CPU/RAM/disk/uptime.',inputSchema:{type:'object',properties:{device_id:{type:'string'}},required:['device_id']}},
{name:'reboot_device',description:'Reboot selected device.',inputSchema:{type:'object',properties:{device_id:{type:'string'},delay_seconds:{type:'integer'}},required:['device_id']}}
];
...
let result;if(m.method==='initialize')result={protocolVersion:m.params&&m.params.protocolVersion||'2025-06-18',capabilities:{tools:{}},serverInfo:{name:'Jarvis Commander',version:'0.1.0'}};
...
else{const map={run_command:'shell',read_file:'read_file',write_file:'write_file',list_directory:'list_dir',system_status:'system_status',reboot_device:'reboot'},act=map[n];
...
else{const c=queue(a.device_id,act,a),o=await wait(c.id,Math.min(Math.max(a.timeout_ms||60000,5000),125000));result={content:[{type:'text',text:JSON.stringify(o,null,2)}],isError:!o.ok}}}}
...
if(req.method==='GET'&&u.pathname==='/health')return send(res,200,{ok:true,name:'Jarvis Commander Relay',version:'0.1.0',devices:devices().length});
...
if(req.method==='POST'&&u.pathname==='/api/command'){if(!admin(req,u))return send(res,401,{error:'unauthorized'});const b=await read(req);if(!state.devices[b.deviceId])return send(res,404,{error:'unknown device'});return send(res,202,{command:queue(b.deviceId,b.action,b.args)})}
...
}catch(e){send(res,500,{error:String(e.message||e)})}}).listen(cfg.port||8792,cfg.host||'127.0.0.1',()=>console.log('Jarvis Commander Relay online'));